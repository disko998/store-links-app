export const stories = [
    {
        id: '1',
        image:
            'https://is2-ssl.mzstatic.com/image/thumb/Purple118/v4/21/90/af/2190af8a-f8af-bf2a-513c-a017aacd9875/source/512x512bb.jpg',
    },
    {
        id: '2',
        image:
            'https://www.rubber-siliconehoses.com/images/brend/scania-logo.jpg',
    },
    {
        id: '3',
        image:
            'https://scontent.fbeg4-1.fna.fbcdn.net/v/t1.0-9/78416290_3122419527788022_273422411246862336_n.jpg?_nc_cat=1&_nc_sid=09cbfe&_nc_ohc=7coZXQdjv4oAX-ET2Hx&_nc_ht=scontent.fbeg4-1.fna&oh=60b1855ff0d226be79dc715ad0519a4d&oe=5F4D2C91',
    },
    {
        id: '4',
        image:
            'https://media-exp1.licdn.com/dms/image/C560BAQGwCcUmtb3InQ/company-logo_200_200/0?e=2159024400&v=beta&t=abCn1Fv1yqLzCEIriQa3Ji73tCDOaaEWEILRi9OhfEI',
    },
]

export const categories = [
    { title: 'my list', active: true, favorite: false },
    { title: 'restaurants', active: false, favorite: false },
    { title: 'cafes', active: false, favorite: false },
    { title: 'pet shop', active: false, favorite: false },
    { title: 'shop', active: false, favorite: false },
]

export const stores = [
    {
        id: '1',
        name: 'H&S',
        details: 'Lorem Ipsum is simply dummy text',
        website:
            'https://hsstore.com/Default.aspx?currency=kwd&device=web&lan=en',
        phone: 123456789,
        favorite: false,
        unique: false,
        image:
            'https://scontent.fbeg4-1.fna.fbcdn.net/v/t1.0-9/54518493_2246086942386173_2424004418589425664_o.jpg?_nc_cat=102&_nc_sid=dd9801&_nc_ohc=6bwWA1gdF48AX-SoTGy&_nc_ht=scontent.fbeg4-1.fna&oh=b47f853766c535d16fc928009e676d80&oe=5F4BD79A',
        logo:
            'https://is2-ssl.mzstatic.com/image/thumb/Purple118/v4/21/90/af/2190af8a-f8af-bf2a-513c-a017aacd9875/source/512x512bb.jpg',
    },
    {
        id: '2',
        name: 'Euphoria',
        details: 'Lorem Ipsum is simply dummy text',
        website: 'https://euphoria.cafe',
        phone: 123456789,
        favorite: true,
        unique: true,
        image:
            'https://media-cdn.tripadvisor.com/media/photo-s/17/5d/9d/86/sunny-patio-exterior.jpg',
        logo:
            'https://euphoria.cafe/uploads/images/4d6e34f8634da8e988b1b1cbe167897c.jpg',
    },
    {
        id: '3',
        name: 'X-cite',
        details: 'Lorem Ipsum is simply dummy text',
        website: 'https://www.xcite.com/',
        phone: 123456789,
        favorite: true,
        unique: false,
        image:
            'https://www.alghanim.com/Alghanim/media/Alghanim-Media/consumer/x-cite-electronics/mobile-banner.jpg?ext=.jpg',
        logo:
            'https://scontent.fbeg4-1.fna.fbcdn.net/v/t1.0-9/78416290_3122419527788022_273422411246862336_n.jpg?_nc_cat=1&_nc_sid=09cbfe&_nc_ohc=7coZXQdjv4oAX-ET2Hx&_nc_ht=scontent.fbeg4-1.fna&oh=60b1855ff0d226be79dc715ad0519a4d&oe=5F4D2C91',
    },
    {
        id: '4',
        name: 'Mr Baker',
        details: 'Lorem Ipsum is simply dummy text',
        website: '',
        phone: 123456789,
        favorite: false,
        unique: true,
        image:
            'https://cdn-a.william-reed.com/var/wrbm_gb_food_pharma/storage/images/9/9/1/6/1776199-1-eng-GB/Coles-fresh-bread-claims-Fined-A-2.5m.jpg',
        logo:
            'https://media-exp1.licdn.com/dms/image/C560BAQGwCcUmtb3InQ/company-logo_200_200/0?e=2159024400&v=beta&t=abCn1Fv1yqLzCEIriQa3Ji73tCDOaaEWEILRi9OhfEI',
    },
    {
        id: '5',
        name: 'Pick',
        details: 'Lorem Ipsum is simply dummy text',
        website: 'http://pick.com.kw/',
        phone: 123456789,
        favorite: true,
        unique: true,
        image:
            'https://pick.com.kw/test/wp-content/uploads/2015/11/qortuba.jpg',
        logo: 'http://pick.com.kw/wp-content/uploads/2015/11/homebglogo.png',
    },
    {
        id: '6',
        name: 'Nabta',
        details: 'Lorem Ipsum is simply dummy text',
        website: '',
        phone: 123456789,
        favorite: false,
        unique: false,
        image:
            'https://lh3.googleusercontent.com/proxy/z9Fz8KVRl00XfFK5I_cti_1N6PmZzPPgVFlBanIUdCT1UNWsb8HcdNI7GeHVwTg_HVaqb0nD8uWomHMI6W3fw_y8bqfzSO9dzMRjwlO9p6TrpvYI',
        logo:
            'https://mostaql.hsoubcdn.com/uploads/333859-1511020787-IMG-20171115-WA0015.jpg',
    },
]

export const seed = [
    {
        name: 'John Doe',
        title: 'H&S',
        order_link:
            'https://hsstore.com/Default.aspx?currency=kwd&device=web&lan=en',
        call_number: '123456789',
        whatsApp_number: '123456789',
        unique: false,
        locations: [],
        categories: [],
        image:
            'https://scontent.fbeg4-1.fna.fbcdn.net/v/t1.0-9/54518493_2246086942386173_2424004418589425664_o.jpg?_nc_cat=102&_nc_sid=dd9801&_nc_ohc=6bwWA1gdF48AX-SoTGy&_nc_ht=scontent.fbeg4-1.fna&oh=b47f853766c535d16fc928009e676d80&oe=5F4BD79A',
        logo:
            'https://is2-ssl.mzstatic.com/image/thumb/Purple118/v4/21/90/af/2190af8a-f8af-bf2a-513c-a017aacd9875/source/512x512bb.jpg',
    },
    {
        name: 'John Doe',
        title: 'Euphoria',
        order_link: 'https://euphoria.cafe',
        call_number: '123456789',
        whatsApp_number: '123456789',
        unique: true,
        locations: [],
        categories: [],
        image:
            'https://media-cdn.tripadvisor.com/media/photo-s/17/5d/9d/86/sunny-patio-exterior.jpg',
        logo:
            'https://euphoria.cafe/uploads/images/4d6e34f8634da8e988b1b1cbe167897c.jpg',
    },
    {
        name: 'John Doe',
        title: 'X-cite',
        order_link: 'https://www.xcite.com/',
        call_number: '123456789',
        whatsApp_number: '123456789',
        unique: true,
        locations: [],
        categories: [],
        image:
            'https://www.alghanim.com/Alghanim/media/Alghanim-Media/consumer/x-cite-electronics/mobile-banner.jpg?ext=.jpg',
        logo:
            'https://scontent.fbeg4-1.fna.fbcdn.net/v/t1.0-9/78416290_3122419527788022_273422411246862336_n.jpg?_nc_cat=1&_nc_sid=09cbfe&_nc_ohc=7coZXQdjv4oAX-ET2Hx&_nc_ht=scontent.fbeg4-1.fna&oh=60b1855ff0d226be79dc715ad0519a4d&oe=5F4D2C91',
    },
    {
        name: 'John Doe',
        title: 'Mr Baker',
        order_link: '',
        call_number: '123456789',
        whatsApp_number: '123456789',
        unique: true,
        image:
            'https://cdn-a.william-reed.com/var/wrbm_gb_food_pharma/storage/images/9/9/1/6/1776199-1-eng-GB/Coles-fresh-bread-claims-Fined-A-2.5m.jpg',
        logo:
            'https://media-exp1.licdn.com/dms/image/C560BAQGwCcUmtb3InQ/company-logo_200_200/0?e=2159024400&v=beta&t=abCn1Fv1yqLzCEIriQa3Ji73tCDOaaEWEILRi9OhfEI',
    },
    {
        name: 'John Doe',
        title: 'Pick',
        order_link: 'http://pick.com.kw/',
        call_number: '123456789',
        whatsApp_number: '123456789',
        unique: true,
        locations: [],
        categories: [],
        image:
            'https://pick.com.kw/test/wp-content/uploads/2015/11/qortuba.jpg',
        logo: 'http://pick.com.kw/wp-content/uploads/2015/11/homebglogo.png',
    },
    {
        name: 'John Doe',
        title: 'Nabta',
        order_link: '',
        call_number: '123456789',
        whatsApp_number: '123456789',
        unique: false,
        locations: [],
        categories: [],
        image:
            'https://lh3.googleusercontent.com/proxy/z9Fz8KVRl00XfFK5I_cti_1N6PmZzPPgVFlBanIUdCT1UNWsb8HcdNI7GeHVwTg_HVaqb0nD8uWomHMI6W3fw_y8bqfzSO9dzMRjwlO9p6TrpvYI',
        logo:
            'https://mostaql.hsoubcdn.com/uploads/333859-1511020787-IMG-20171115-WA0015.jpg',
    },
]
